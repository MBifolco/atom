...
class TestTerminationRewards:
    ...
    def test_win_reward_basic(self):
        ...
        total_reward = 0
        for _ in range(300):
            action = np.array([1.0, 0.7], dtype=np.float32)
...
class TestTruncationRewards:
    ...
    def test_truncation_occurs_at_max_ticks(self):
        ...
        tick_count = 0
        for _ in range(25):
            action = np.array([0.0, -0.7], dtype=np.float32)  # Defend
            obs, reward, terminated, truncated, info = env.step(action)
            tick_count += 1
            if terminated or truncated:
                break
...
class TestMidEpisodeRewards:
    ...
    def test_damage_reward_when_hitting(self):
        ...
        rewards = []
        for _ in range(20):
            action = np.array([1.0, 0.7], dtype=np.float32)  # Attack
            obs, reward, terminated, truncated, info = env.step(action)
...
class TestStanceSelection:
    ...
    def test_stance_clamping(self):
        ...
        # Action with extreme selector values should still work
