...
class TestTerminalRewardBranches:
    ...
    def test_win_with_quick_time_bonus(self):
        ...
        for _ in range(100):
            action = np.array([1.0, 0.7])  # Max attack
            obs, reward, done, truncated, info = env.step(action)

            if done:
                # Win reward should include time bonus
                assert reward > 5.0, f"Quick win should have bonus (terminal /10), got {reward}"
                break
    ...
    def test_win_with_stamina_efficiency_bonus(self):
        ...
        for i in range(120):
            ...
            if i % 3 == 0:
                ...
                action = np.array([0.3, 0.7])  # Attack
            obs, reward, done, truncated, info = env.step(action)
    ...
    def test_loss_with_hp_penalty(self):
        ...
        for _ in range(250):
            action = np.array([0.0, -0.7])  # Just defend
            obs, reward, done, truncated, info = env.step(action)

            if done:
                # Loss should give negative reward
                if obs[2] == 0:  # We died
                    assert reward < -5.0, f"Loss should be heavily penalized (terminal /10), got {reward}"
                break
...
class TestMidEpisodeRewardBranches:
    ...
    def test_stamina_advantage_reward(self):
        ...
        for _ in range(25):
            action = np.array([0.0, -0.7])  # Defend to save stamina
            env.step(action)
    ...
    def test_low_stamina_fighting_penalty(self):
        ...
        for i in range(50):
            if i < 25:
                action = np.array([1.0, 0.7])  # Exhaust stamina
            else:
                action = np.array([0.5, 0.7])  # Keep attacking despite low stamina
            env.step(action)
    ...
    def test_proximity_closing_when_opp_weak(self):
        ...
        for i in range(60):
            if i < 20:
                action = np.array([1.0, 0.7])  # Attack to lower opponent HP
            else:
    ...
    def test_proximity_backing_off_when_low_stamina(self):
        ...
        for i in range(45):
            if i < 20:
                action = np.array([1.0, 0.7])  # Exhaust
            else:
                direction_away = -1 if i % 2 == 0 else 1
                action = np.array([direction_away * -0.8, -0.7])  # Back away and defend
            env.step(action)
    ...
    def test_stance_extended_when_opp_hurt(self):
        ...
        for _ in range(55):
            action = np.array([0.7, 0.7])  # Extended stance
            env.step(action)
    ...
    def test_stance_defending_when_low_stamina(self):
        ...
        for i in range(40):
            if i < 18:
                ...
                action = np.array([0.0, -0.7])  # Defend with low stamina
            env.step(action)
